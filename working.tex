...
A system architect must model their workflows as they are intended to be
executed in practice, but their organisational context introduces contingent behaviour which is
fundamental to the workflow's accurate simulation and proves cumbersome to model with
existing techniques.  Conventional systems engineering notations for describing user behaviour, such as BPMN
...
\item A method for simulating user interactions with information systems that allows for the separate modelling of
  ...
  \citep{takanen08fuzzing}. It is demonstrated that this combination allows for
  the flow of workflow execution to be altered without sacrificing readability
  or simplicity.
...
encapsulated within coarser grained modules.  As argued in \Cref{sec:introduction}, user behaviours are
...
behaviour on the overall system. The observation made in \Cref{sec:introduction} that the causes of irregularities in
...
as a collection of Python classes.  \Cref{fig:domain} shows the class diagram
...
is implemented transparently with respect to workflow classes, easing the modelling burden.  The execution
of tasks and their duration is logged in a tree-like structure by each agent, supporting later inspection and analysis
of behaviour.
