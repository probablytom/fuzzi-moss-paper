...
for a maximum of 500 simulation clock ticks.  Each configuration was run for 25 different projects.  Both the Waterfall
and TDD SDLC workflows were simulated.  In either case, the incomplete procedure distraction aspect was applied to
either the high level software development workflow, or to the set of lower level set of development activity workflows
(implementation, change management etc.).  The distraction fuzzing aspect was applied using developer concentration
values between 0.001 and 5.0.  Software projects consisted of up between 1 and 6 features, with projects divided into
small (2 chunks per feature) and large (4 chunks per feature).

The average number of features completed during the 25 runs of a simulated software development effort was recorded to
measure the productivity of a simulation.  To measure the quality of the development built by the simulation, the system
was operated 10 times through random selection and operation of features for up to 750 features.  If a bug is manifested
during operation then the system halts.  The average trace of the 10 operations of the system was recorded as the
system's mean time to failure (MTF).  Results from the simulations are available for inspection
...
\end{figure}
In both plots, the number of features implemented for Waterfall projects is plotted in blue TDD in red against the total
number of statements removed by fuzzing.  Figure \ref{fig:fuzzing-features:WT} shows the effect when only the high level
...
a far steeper decline for Waterfall than for TDD.  In Figure \ref{fig:fuzzing-features}, both SDLC workflows appear to
be largely immune to distraction up to the removal of approximately 100 statements, after which feature completion
