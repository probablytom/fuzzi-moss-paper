...
users. As a result, these models must choose to what degree they will contend
with unexpected behaviour. Models increase in their complexity as more caveats
are included.
However, different behaviours often vary due to the same underlying factors: a single flaw in a real-world agent's 
execution of a workflow can manifest in myriad ways. This paper shows that an agent's variance is a cross-cutting
concern which can be separated from the workflows they follow. This is explored through the development and application
of a modelling tool which introduces variance to an idealised workflow model,
where the flaw in an agent's behaviour is modelled as opposed to its realisation
in the workflow.
\end{abstract}
...
\todo{I wondered whether it was worth mentioning something about Somerville's
  Large-Scale Complex IT System work here, which looked at the \$800bn lost to
  sociotechnical factors on the Dow Jones, but didn't want to confuse our
  message...? }
...
\item A demonstration of the efficacy of the framework in a case study comparison of Test Driven
  Development (TDD) and Waterfall software development workflows.  The case study provides evidence for the industry consensus and
  ...
  \todo{I think we can remove this, so I've moved it into a Todo note...}
  \todo{Specifically, the simulation results show that: the compared workflows provide for similar outcomes in idealised
...
techniques for modeling\todo{modeling or modelling? Seems modeling is US?} socio-technical workflows and the limitations encountered in the literature.  Section
...
Nets\todo[color=cyan]{Really? I thought they were based on flow charts? --Tom.}, as is BPMN, but with a richer notation
...
to the set of lower level development activity workflows (implementation, change management etc.).  The
...
development strategy\todo{do you suppose this wants a citation? Maybe something we already have fits.}, although little empirical evidence exists to support this consensus.  Two aspects of the effect of
...
Similar to the plots of feature completion, the plots of MTF suggest that TDD is
more resilient to deviations from ideal developer behaviour than
Waterfall, which experiences a much more dramatic decline as the number of statements removed increases.  This supports
the contention of advocates that TDD results in higher quality systems because of the explicit quality assurance throughout
...
\todo{This is literally the only time FM is mentioned in the paper --- do we
  want to refer to FM in section 4 or change this to pydysofu?}
...
\todo{I have to leave the machine as I'm writing this but I'll come back and
  write something here referring back to what we fix in the relevant
  ltierature's shortcomings. Might as well prove that this is contributing
  something novel and this feels like a good palce to do it!}
i\todo{I'll also come back maybe to refer to uses in simulations of
  human-centered security and safety critical systems. Maybe simulations of
  complex adaptive systems too? Unless that's covered by one of the existing
  things we reference in this para.}
