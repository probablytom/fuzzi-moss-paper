...
\item A method for simulating user interactions with information systems that allows for the separate modelling of
  idealised descriptions of workflows and the effects of irregularities on those workflows.  The approach is implemented in
  a fuzzing aspects software framework, using a combination of aspect oriented programming \cite{filman01aspect} and
  dynamic code fuzzing \citep{takanen08fuzzing}, allowing the flow of execution in a workflow description
  to be altered without sacrificing readability or simplicity.
...
of simulating realistic user behaviour. This approach combines dynamic software fuzzing to provide variation in the
workflow model; and aspect oriented programming (AOP) to decouple the behavioural variation from the idealised model.
