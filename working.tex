...
behaviour on the overall system.  Our approach, in recognising that the causes of irregularities in a workflow as cross
cutting concerns leverages methods established in Software Engineering. The representation of cross cutting concerns
...
implemented as collection of Python classes.  \Cref{fig:domain} shows the class diagram for the problem domain in
...
first class in \Cref{fig:workflows}.  The state for the workflow is the target version control server and a client
...
can also be organised hierarchically.  \Cref{fig:workflows} also shows this modularity is exploited for the
